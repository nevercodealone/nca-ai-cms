    ...
    for (const post of duePosts) {
      try {
        if (!post.generatedTitle || !post.generatedContent) {
          console.warn(`[AutoPublisher] Skipping ${post.id}: missing generated content`);
          failed++;
          continue;
        }
        ...
        published++;
        console.log(`[AutoPublisher] Published ${post.id} -> ${article.folderPath}`);
      } catch (error) {
        failed++;
        console.error(`[AutoPublisher] Failed to publish ${post.id}:`, error);
      }
    }

    console.log(`[AutoPublisher] Done: ${published} published, ${failed} failed`);
  } catch (error) {
...
export function startAutoPublisher(): void {
  ...
  console.log(`[AutoPublisher] Starting (interval: ${INTERVAL_MS / 1000 / 60} minutes)`);
  ...
  intervalId = setInterval(publishDuePosts, INTERVAL_MS);
}
